...
preinstall(){
	...
	color cyan "'++:.                           '-/+/" >&2
	color cyan ".'                                 '" >&2
	color deepblue "Welcome to CryptonoteOS-Wallet Version 0.01.">&2
...
postinstall(){
    ...
    arch-chroot /mnt pacman -S --noconfirm git base-devel cmake miniupnpc unbound graphviz doxygen libunwind pkg-config openssl curl gtest base-devel cmake wget
    arch-chroot /mnt wget wget https://sourceforge.net/projects/boost/files/boost/1.68.0/boost_1_68_0.tar.bz2
    arch-chroot /mnt tar xvfo boost_1_68_0.tar.bz2
    arch-chroot /mnt cd boost_1_68_0
    arch-chroot /mnt ./bootstrap.sh
    arch-chroot /mnt ./b2
    arch-chroot /mnt ./bjam cxxflags=-fPIC cflags=-fPIC -a install
    arch-chroot /mnt cd ..
    ...
    color default "----------------------------------------------------------------------"
    color cyan "Now just boot into system and launch your coin daemon for sync"
}
